...
<th> @lang('view_pages.uuid')
<span style="float: right;">
</span>
</th>
<th> @lang('view_pages.owner')
<span style="float: right;">
</span>
</th>
...
<button type="button" class="btn btn-info btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">@lang('view_pages.action')
    ...
    <div class="dropdown-menu">
        {{-- @if (auth()->user()->can('driver-profile')) --}}
        <a class="dropdown-item grey-text text-darken-2" href="{{url('company/drivers/profile',$result->id) }}">{{ trans('view_pages.profile')}}</a>
        {{-- @endif --}}
        
       <a class="grey-text text-darken-2" href="{{url('company/drivers/vehicle/privileges',$result->id) }}">{{ trans('view_pages.vehicle_privileges')}}</a>

        {{-- @if (auth()->user()->can('approve-disapprove-driver')) --}}
        @if ($result->approve)
        <a class="dropdown-item sweet-decline grey-text text-darken-2" href="{{url('company/drivers/toggle_approve',$result->id) }}">{{ trans('view_pages.block')}}</a>
        @else
        <a class="dropdown-item sweet-approve grey-text text-darken-2" href="{{url('company/drivers/toggle_approve',$result->id) }}">{{ trans('view_pages.approve')}}</a>
        @endif
                                                {{-- @endif --}}
