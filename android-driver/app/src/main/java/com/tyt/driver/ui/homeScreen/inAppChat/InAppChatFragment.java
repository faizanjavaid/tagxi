...
import com.tyt.driver.databinding.FragmentInAppChatBinding;
...
//                Toast.makeText(getBaseAct(), "You can't send empty message", Toast.LENGTH_SHORT).show();
    ...
    public void onReceivedChatShow(JsonObject jsonObject) {
        ...
        chatAdapter.smsNotify(chatList);
        for (int i = 0; i < chatList.size(); i++) {
            Log.d("xxxTAG", "onReceivedChatShow: index=" + i + " " + "message =" + chatList.get(i).getMessage() + " status =" + chatList.get(i).getMessageStatus());
        }
